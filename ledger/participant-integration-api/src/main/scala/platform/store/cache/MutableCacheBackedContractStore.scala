...
class MutableCacheBackedContractStore(
  ...
  private[cache] val cacheEventSequentialId = new AtomicLong(EventSequentialId.beforeBegin)
  private[cache] val cacheEventOffset = new AtomicReference(Offset.beforeBegin)
  ...
  private val (contractStateUpdateKillSwitch, contractStateUpdateDone) =
    RestartSource
      .withBackoff(
        ...
        subscribeToContractStateEvents(cacheEventOffset.get(), cacheEventSequentialId.get())
  ...
  private def readThroughContractsCache(contractId: ContractId)(implicit
    ...
    val currentCacheOffset = cacheEventSequentialId.get()
  ...
  private def readThroughKeyCache(
    ...
    val currentOffset = cacheEventSequentialId.get()
  ...
  private def updateOffsets(event: ContractStateEvent): Unit = {
    cacheEventOffset.set(event.eventOffset)
    cacheEventSequentialId.set(event.eventSequentialId)
