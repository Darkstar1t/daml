...
alter table participant_events
    alter column submitter type NVARCHAR2(1000) array using array[submitter];
alter table participant_events
    rename column submitter to submitters;

alter table participant_command_completions
    alter column submitting_party type NVARCHAR2(1000) array using array[submitting_party];
alter table participant_command_completions
